...
// const odds = Object.values(game.odds);
// for (const odd of odds) console.log( `Odd of victory ${}` );;
...
//   scorers[player];
//   scorers[Object.toString(player)] += 1;
// }
// console.log(scorers);
// GOOD LUCK 😀
...
// // console.log(times);
// // let averageEventTime = 0;
// // for (const event of times) {
// //   averageEventTime += event;
// // }
// // averageEventTime /= 90;
...
document.querySelector('button').addEventListener('click', function () {
  ...
  //ittering through array of words
  for (let [index, word] of words.entries()) {
    // identyfying separate words
    const [firstWord, lastWord] = word.trim().toLowerCase().split('_');
    // Upercase
    const upperLastWord = lastWord.replace(
      lastWord[0],
      lastWord[0].toUpperCase()
    );
    // joining words
    const camelCaseWord = firstWord + upperLastWord;

    //padding
    const paddedCamelCase =
      camelCaseWord.padEnd(20, '.') + tick.repeat(index + 1);
    // ticks
    paddedCamelCase;
    // printing out
    console.log(paddedCamelCase);
  }
});
